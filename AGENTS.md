...
- The app **compiles** on Windows (`zig build` → `zig-out/bin/nova.exe`); full runtime support is still in progress and tracked in issues #24–#27 (config paths, bash shell, failing tests, ModernBERT worker / lanes). Linux behavior is unchanged.
...
- **Ollama `reasoning_effort` value clipping (chat-completions + Responses API).** Ollama's `/v1/chat/completions` validates `reasoning_effort` strictly: only `high`/`medium`/`low`/`max`/`none` are accepted — `xhigh`/`minimal` return HTTP 400. Both invalid values are reachable via the global hardcoded reasoning-options list (`tui.zig`). `wireEffortLabel` (`openai_compatible.zig`, `pub`) clips them for the `.minimal` dialect (`xhigh→"max"`, `minimal→"low"`); `.openai` keeps raw labels (gpt-5 honours both). The helper is called from BOTH wire clients: `openai_compatible.writeRequestPayload` (flat `reasoning_effort`) and `responses_core.writeRequestPayload` (`reasoning.effort` object). The Responses API path (`useResponsesEndpoint: true`, opt-in) was initially a bypass — Ollama's `/v1/responses` silently ignores unknown reasoning fields (Go's default JSON decoder drops them, and the endpoint defines no reasoning struct), so it wasn't crash-causing, but clipping keeps the two clients consistent and is forward-safe. The `include:["reasoning.encrypted_content"]` emitted unconditionally by `responses_core` when reasoning is set is also silently ignored by Ollama's `/v1/responses` (unsupported, no struct field) — left as-is because it's harmless noise and suppressing it for `.minimal` would require broader dialect-gating of the Responses API client.
