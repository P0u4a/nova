...
- **Session resume segfault (null `model_id`).** Crash at `src/ai/openai_compatible.zig:59` (`gpa.dupe(u8, config.model)`) when `summary.model_id` is null and flows into `ModelSelection.custom.model.id` (non-optional `[]u8`). Resume chain: `root.run` → `initResume` → `initSession` → `applyFromConfig` → `tryAttachOpenAiCompatibleFromConfig` → `attachOpenAiCompatibleClient` → `Client.init`. Guarded at three entry points: `runtime.applyFromConfig` skips attachment when `model_id.len == 0`, `tui.applySelectedModel` returns `error.EmptyModelId` before constructing the selection, and `writeRequestPayload` returns `error.EmptyModelId` on an empty model (H1) — a real early return that survives into ReleaseFast where the old `std.debug.assert` was stripped to `unreachable` (UB). Always use a real early return for these guards, never `std.debug.assert` — `unreachable` is UB in ReleaseFast (the install target) and would not protect that build. Previous fix: `da7c761` ("guard empty model_id on resume").
...
- **Approval gates on background lanes are visible (fixed 2026-08-05).** `permission.approvalLane(app)` scans all lanes for a pending gate (visible lane first), so a worker blocked in `ApprovalGate.request` surfaces in the overlay — labeled "Lane {id} requests approval" when the owner isn't the visible lane — and `resolvePermission` routes to the owning lane. `laneStatus` reports "running — waiting for approval" before the stall check, so `lane list`/`read`/`await` show an honest signal instead of a fake STALLED. This closes plan-doc review-pass limitation #7.

- **Lane completion honesty (fixed 2026-08-05).** (a) `deliverPendingLaneCompletions` now enqueues the result to the spawner's agent queue *first*, then appends the transcript notice, then sets `completion_delivered` — on `QueueFull` nothing has been written and the next tick retries, so a worker's result is never dropped while a notice claims delivery. (b) `handleInterrupt` and `cancelLaneTurn` now record the interrupt into `turn_failed` (a second dupe of `cancel_message`; the event's copy is freed by `event.deinit`), so a cancelled worker's completion reports "FAILED — Interrupted." instead of "final state: done". A fresh turn's `resetTurnState` clears it.

- **`spawned_by_agent` replaced by a generation counter (fixed 2026-08-05).** `Thread` now carries `generation` (assigned from `App.lane_generation_counter` at every creation site) and `spawned_by_generation` (set at spawn); completion delivery routes via `App.laneByGeneration` instead of the raw `*Agent` pointer comparison. A session switch that frees the spawner's agent can no longer misroute a completion via allocator address reuse. `laneForAgent` stays for `handleLaneRequest`, which uses a *live* requester pointer (no ABA hazard).

- **`mergeLaneOp` reports the driver's real workspace (fixed 2026-08-05).** It now replies with `driverWorkspace(app) orelse repo` — when the driver holds a workspace borrow in another lane (H5b only refuses merging the *entered* lane itself), the response names that lane's path instead of falsely claiming the repo root.

- **`turn_lifecycle.zig` frees `pending_prompt` before the delivery turn (fixed 2026-08-05).** `restartTurnForQueuedMessages` / `startDeliveryTurnOnCurrentThread` now free any `beginSubmit`-duped prompt (and pass an explicit `null`) instead of null-then-pass — messages travel via `drain_queue_first = true`, so no prompt is handed over, and a prompt left over from a failed `startTurn` no longer leaks.

- **`installRuntime` (session switch) checks only the active lane's turn — by design.** Other lanes keep running against their own runtimes/sessions; each lane owns its runtime, so a switch must not tear down lanes the user is still working in. Completion routing is generation-safe across the switch (M1). Documented in `transcript_lifecycle.zig` and `_plan/plan-lane-worker-hardening-2026-08-05.md` (I4).

- **UI queue mirror diverges from the agent queue under `enqueueRaw` (fixed 2026-08-05).** Raw delivery (lane completion, background jobs) goes through `queue.enqueueRawMirrored`, which dual-writes the `raw` mirror entry only AFTER a successful `enqueueRaw` — a QueueFull drop writes nothing on either side, so the mirror stays 1:1 with the agent queue and `setQueuedSteer(index)` — which indexes the agent queue — always marks the right entry. `flushQueuedUserMessagesToTranscript` drops raw entries without rendering them (the delivery path already wrote its own notice). The no-provider branch of `startDeliveryTurnOnCurrentThread` flushes the mirror before `clearQueue` so clearing the agent queue can't strand mirror entries.
- **Text-emitted tool calls are recovered pre-persist (T1/T2/T3, added 2026-08-06).** Some function-calling models (ling-3.0-flash, weaker gateways) emit valid tool *intent* as literal text inside a `{"type":"text"}` block (`<tool_call>bash<arg_key="command">…</arg_key></tool_call>` or `<tool_call>{"name":…,"arguments":{…}}</tool_call>`) instead of structured `tool_calls` deltas. The stream parser only reconstructs structured deltas, so such a turn has zero `.tool_call` blocks and the agent goes idle as if the model said nothing. Recovery (`recoverTextToolCalls` / `injectRecoveredToolCalls` in `src/agent.zig`, parser in `src/ai/text_tool_call.zig`) runs in the narrow window BETWEEN `collectToolCalls` and `takeAssistantMessage` in the turn loop (agent.zig `runPromptedTurn`) — it MUST run there because `takeAssistantMessage` invalidates `turn.assistant` (`= undefined`), and `hasWireContent` is `true` for a text-only message, so by the `len == 0` site the message is already gone. Ownership: `recoverTextToolCalls` returns freshly-allocated `ToolCall`s (their `call_id`/`name`/`arguments` are minted in the parser), `injectRecoveredToolCalls` appends them as new `.tool_call` ContentBlocks reallocating the content slice, and the SAME `recovered` slice drives execution (no re-collect). This mirrors `collectToolCalls`'s aliasing contract (slice shell caller-freed; element strings belong to the message). Conservatism guards: only fires when structured calls are zero, only scans `.text` blocks, and the parser only matches the explicit `<tool_call>` envelope — free-form angle brackets are never misread. The system prompt (`<tool_calling>` block) also directs compliant models never to emit tool calls as text (T2), so this is a safety net for models that ignore the directive.
- **Request-body logging surfaces the `tools` array past the head (L1, added 2026-08-06).** `logBytes` (`openai_compatible.zig`) used to truncate at 12KB head-only; because JSON key order is `model → messages → … → tools`, a realistic system prompt + a few turns already consumed the head, hiding the `tools` array (the most-wanted field when debugging tool-calling). It now appends a tail slice starting at `"tools":` when truncating, so the array is visible without reverse-engineering the payload from test fixtures. Callers must `defer`-free only when `result.ptr != bytes.ptr` (short bodies alias through).
- **`NO tools` and parallel-limit rejects carry model identity (L2/L3, added 2026-08-06).** The `tools_json == "[]"` warning is emitted from `Client.prompt` (not the free `writeRequestPayload`) so it carries `self.url` + `self.config.model`, disambiguating the tool-less summarizer/naming clients from a tools-less main client. The over-cap tool-call reject (`parseToolCall.reject`) logs the model id — `readStream` takes a borrowed `model` param threaded from `self.config.model`; the Responses-API client's separate local `readStream` has no `max_calls` path so L3 doesn't apply there.
- **`beginSubmit` null-deref on idle/rested lanes (fixed 2026-08-06).** `beginSubmit` (`turn_lifecycle.zig`) unconditionally unwrapped `app.thread.worker_context.?` — null for idle lanes created via `lane create` (`lane_lifecycle.zig`) and for rested lanes (via `parkFinishedWorker`, `lane_lifecycle.zig`, which nulls `worker_context`/`agent`). Crash path: `lane create` → idle lane in grid → Shift+Tab (`cycleLane`, `lane_lifecycle.zig`) sets `app.thread` to the idle lane (no guard) → type + Enter → `submit` (`lifecycle.zig`) → `beginSubmit` → null deref → SIGABRT. `beginSubmit` checks `liveRuntime()` first (null for idle → skips the no-provider branch) then hits `worker_context.?`. The unwrap had never been guarded — it predated lane hardening. Now guarded: `beginSubmit` returns `false` with a guiding transcript notice ("Lane … is idle — `lane enter` to work here, or `lane spawn` to start a worker in it") **before** `toOwnedSlice`, so the user's typed input is preserved (TD-2). No wake-on-submit by design (TD-1).
- **`lane spawn` ignores `req.lane` — cannot target an existing idle lane (fixed 2026-08-06).** `spawnLane` (`lane_lifecycle.zig`) always created a fresh worktree + runtime + `contained = true` lane; it never read `req.lane`. The schema documented `lane` as "unused for list/create/spawn" (`lane.zig`), so the model could not spawn a worker into an existing idle lane created via `lane create`. `spawnLane` had never used `req.lane` in any commit. Now `lane spawn {lane}` targets an existing idle lane (from `lane create` or a rested worker): `wakeIdleLane` attaches a runtime to the idle `Thread` in place, reusing its worktree+branch and adopting the spawner's naming context (TD-3); the transcript is preserved and the new turn appends (TD-5). Null/unresolvable `req.lane` keeps the fresh-worktree path; running/primary lanes are refused (TD-6).
