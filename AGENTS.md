...
**Lua Plugin System pattern.** `src/lua/` implements a full Lua 5.4 plugin SDK. `PluginManager` (manager.zig) discovers plugins from `~/.config/nova/plugins/` (global) and `.nova/plugins/` (project), loads manifests (`plugin.lua`), and creates sandboxed Lua states. Each plugin runs in a restricted environment — `io`, `debug`, `package`, `os.execute` are blocked; instead plugins use 18 `nova.*` bridge functions (read_file, write_file, run_bash, git_status, register_tool, etc.). The `App` struct holds `plugin_manager: lua_mod.PluginManager` initialized in `initRuntime`. Plugin tools are injected into the AI client via `injectPluginTools()` (provider_model.zig) with naming `lua__<plugin>__<tool>`, following the same pattern as MCP tool injection. The `/plugins` TUI overlay shows loaded plugins with active/inactive status. See `docs/plugins/` for the full plugin development guide and API reference.

**Plugin tool injection pattern.** `injectPluginTools()` in `provider_model.zig` mirrors `injectMcpTools()` exactly: it calls `buildPluginToolSchemas()` to collect registered tools from all active plugin Lua states, converts them to `ai.McpToolSchema[]` with `lua__<plugin>__<tool>` naming, and pushes them into the live AI client via `runtime.client.updateMcpTools(schemas)`. This is called from `refreshMcpTools()` so plugin tools are available at connect time and on refresh. The `nova.register_tool(spec)` C function stores tool specs in the Lua registry (`nova_tools` table) with handler references via `luaL_ref`.
...
- `tools.Schema.Property` — `nullable: bool` flag supporting OpenAI strict-mode union types (`["string","null"]`). The schema writer emits `strict: true`, `additionalProperties: false`, and nullable union arrays only when `nullable` is set, while keeping nested object `additionalProperties: true` for free-form fields like `env`.
