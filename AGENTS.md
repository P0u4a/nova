...
**Config layering pattern.** Config merges across three layers: global (`~/.config/nova/config.json`) → project (`.nova/config.json`) → env vars. `mergeLayers` produces `cached_config`; `applyConfigOverlay` then applies runtime changes (model selection, settings) to this merged result. `model_selection` is canonical — if present in the overlay it wins; if absent, legacy fields apply and `syncModelSelectionFromLegacy` mirrors them back. `serialize` skips `api_key` entirely (lives in `auth.json`) and skips `base_url` when `model_selection` is present (rehydrated from providers). `saveSettings` writes only settings-managed fields (`enable_thinking`, `use_responses_endpoint`, `system_prompt`, `bash_classifier_url`) — never provider/model. If a project config exists, settings are written to both global and project configs; otherwise global only. `modelSelectionUpdates` sets both legacy fields and `model_selection` for self-consistency on disk. `defaultModelScope` returns `.project` if project config exists, `.global` otherwise.
