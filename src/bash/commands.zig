/// Purpose: Intercept bash tool calls and handle each unix command in a special way.
///
/// For most commands we just run them via bash as normal. For the recognized
/// utilities (cat, head, tail, sed, ls, wc) we execute in-process with a
/// pipeline walker that hands off to bash at the first unrecognized segment.
...
pub const Error = error{OutOfMemory} || std.Io.Cancelable || std.Io.UnexpectedError;
...
/// the full command through bash. Returns a populated Result when we own
/// execution.
pub fn tryHandle(
...
test "tryHandle intercepts a simple ls" {
    const gpa = std.testing.allocator;
    const cwd = try std.process.currentPathAlloc(std.testing.io, gpa);
    defer gpa.free(cwd);
    var result = (try tryHandle(gpa, std.testing.io, cwd, "ls")).?;
    defer result.deinit(gpa);
    try std.testing.expectEqual(@as(u8, 0), result.code);
    try std.testing.expect(result.stdout.len > 0);
}

test "tryHandle pipes head output into bash for the unrecognized stage" {
    const gpa = std.testing.allocator;
    const cwd = try std.process.currentPathAlloc(std.testing.io, gpa);
    defer gpa.free(cwd);
    // head with no file reads (empty) stdin then exits, producing empty stdout.
    // `tr a-z A-Z` is unrecognized, so we hand off to bash with the empty buffer.
    // bash's tr returns code 0 with empty stdout.
    var result = (try tryHandle(gpa, std.testing.io, cwd, "head -n 5 | tr a-z A-Z")).?;
    defer result.deinit(gpa);
    try std.testing.expectEqual(@as(u8, 0), result.code);
    try std.testing.expectEqualStrings("", result.stdout);
}

test "tryHandle splits on semicolons" {
    const gpa = std.testing.allocator;
    const cwd = try std.process.currentPathAlloc(std.testing.io, gpa);
    defer gpa.free(cwd);
    var result = (try tryHandle(gpa, std.testing.io, cwd, "wc -l ; wc -l")).?;
    defer result.deinit(gpa);
    try std.testing.expectEqual(@as(u8, 0), result.code);
    // Both stages read empty stdin and emit "0\n".
