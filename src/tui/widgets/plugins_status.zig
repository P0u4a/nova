...
pub const Content = struct {
    ...
    fn draw(ptr: *anyopaque, ctx: vxfw.DrawContext) std.mem.Allocator.Error!vxfw.Surface {
        ...
        try panel.lineStyledAt(&surface, 0, "LUA PLUGINS", ctx, 2, StylePalette.panel_header);
        ...
        if (self.plugins.len == 0) {
            try panel.lineStyledAt(&surface, 4, "No plugins loaded. Add plugins to ~/.config/nova/plugins/ or .nova/plugins/.", ctx, 2, StylePalette.thinking_body);
            try panel.lineStyledAt(&surface, height - 2, "[Esc] Close", ctx, 2, StylePalette.thinking_body);
            return surface;
        }
