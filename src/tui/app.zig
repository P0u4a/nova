// Local vxfw app loop adapted from libvaxis' vxfw.App.
// Nova owns this copy so resize behavior can avoid an immediate terminal clear.
...

        {
            ...
            while (loop.queue.drain()) |event| {
                ...
                switch (event) {
                    ...
                    .winsize => |ws| {
                        try self.resizeWithoutTerminalClear(ws);
                        ctx.redraw = true;
                    },
                    else => {
                        try focus_handler.handleEvent(&ctx, event);
...
fn resizeWithoutTerminalClear(self: *App, winsize: vaxis.Winsize) !void {
    self.vx.screen.deinit(self.allocator);
    self.vx.screen = try vaxis.Screen.init(self.allocator, winsize);
    self.vx.screen.width_method = self.vx.caps.unicode;
    self.vx.screen_last.deinit(self.allocator);
    self.vx.screen_last = try vaxis.AllocatingScreen.init(self.allocator, winsize.cols, winsize.rows);
