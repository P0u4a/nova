...
fn runBackgroundImpl(
    ...
    const text = try std.fmt.allocPrint(
        ...
        "Started in the background as {s} (pid {d}).\n" ++
            "Output is streaming to {s} — read that file (e.g. Get-Content -Tail) or use process commands to check on it.\n" ++
...
/// the model-facing observation. Shared with the bash tool.
