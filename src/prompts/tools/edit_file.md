...
- **Whole block replace/delete**: pick the range so it spans both halves of the brace pair — start on the line that ends with `{`, end on the matching `}`. For pure removal use `- A..B`; for replacement, the payload's first line ends with `{` and last line is the matching `}`.
...
- Do not wrap the patch in Markdown fences. Lines beginning with ``` are treated as normal patch/content lines and will fail unless they are payload lines starting with `{{hsep}}`.
