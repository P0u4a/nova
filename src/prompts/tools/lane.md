...
| `spawn` | `task` | start an independent worker agent in a fresh lane (the task is its first prompt) |
...
- `lane enter` — re-root your tools into that lane's worktree. After entering, bash/file calls run against the lane checkout and the `@file` mention reads files from the lane. `git` inside is on the lane's branch.
...
- `lane spawn` — start an independent worker agent in a fresh lane with the given task. The worker runs on its own thread, concurrently with you; its result arrives as a message. Use it two ways: **fan out** (one lane per independent unit — PRs, candidates, scans) or **staged pipelines** (spawn stage 1, await its result, then spawn stage 2 with that result embedded in the task). Every task must be self-contained — exact paths/branches, what to do, what to report — because the worker starts fresh and can't see your reasoning. After spawning, keep working: results arrive as messages, and `lane read` shows progress; only `lane await` when your next step actually needs that worker's result. Collect with `lane read {lane}` / `lane await {lane}`, then `lane merge {lane}`.
