...
pub const Client = struct {
    ...
    pub fn init(target: *Client, gpa: std.mem.Allocator, io: std.Io, config: ai.Config) !void {
        ...
        const url = try std.fmt.allocPrint(gpa, "{s}/v1/responses", .{std.mem.trimEnd(u8, config.base_url, "/")});
    ...
    pub fn prompt(self: *Client, messages: []const ai.ChatMessage, observer: ai.StreamObserver) !ai.Turn {
        ...
        try writeRequestPayload(&body_writer.writer, self.config.model, messages, self.tools_json, self.config.reasoning);
        try body_writer.end();
...
fn writeRequestPayload(out: *std.Io.Writer, model: []const u8, messages: []const ai.ChatMessage, tools_json: []const u8, reasoning: ?ai.Reasoning) !void {
...
fn readStream(gpa: std.mem.Allocator, reader: *std.Io.Reader, observer: ai.StreamObserver, call_seq: *u64) !ai.Turn {
    ...
    while (try readStreamLine(gpa, reader)) |line| {
        ...
        try processEvent(gpa, data, &blocks, &tools, observer, call_seq, &completed);
    }
    if (completed) {}
